...
const ViewExpensesByMonth = () =>{
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
          const response = await axios.post('http://127.0.0.1:5000/viewExpensesByMonth', {
            email,
            month
          }, {
            headers: {
          ...
          if (response && response.data) {
            if(response.data.tableData.content.length===0) setError('No transactions found for this user');
            else setError('');
            setEmail('');
            setMonth('');
...
export default ViewExpensesByMonth;